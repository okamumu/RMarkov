...
context("Unit tests for blas of COO")

# this test requires src/test_blascoo.cpp
...
dense_to_sparse <- function(x) {
  as(Matrix(x), "TsparseMatrix")
}

test_that("blas coo dgemvN", {
  alpha <- runif(1)
  beta <- runif(1)
  ...
  C_dgemvN_coo(alpha, spA, x, beta, y)
  expect_equal(y, z)
})

test_that("blas coo dgemvT", {
  alpha <- runif(1)
  beta <- runif(1)
  ...
  C_dgemvT_coo(alpha, spA, x, beta, y)
  expect_equal(y, z)
})

test_that("blas coo dgerN", {
  alpha <- runif(1)
  m <- 20
  n <- 10
  ...
  C_dgerN_coo(alpha, x, y, spA)
  expect_equal(spA, spZ)
})

test_that("blas coo dgerT", {
  alpha <- runif(1)
  m <- 20
  n <- 10
  x <- runif(m, min = -10, max = 10)
  ...
  z <- alpha * X + A
  spZ <- dense_to_sparse(z)
  C_dgerT_coo(alpha, y, x, spA)
  expect_equal(spA, spZ)
})

test_that("blas coo dgemmNN", {
  m <- 20
  n <- 10
  k <- 5
  alpha <- runif(1)
  ...
  z <- alpha * (A %*% B) + beta * C
  C_dgemmNN_coo(alpha, spA, B, beta, C)
  expect_equal(C, z)
})

test_that("blas coo dgemmTN", {
  m <- 20
  n <- 10
  k <- 5
  alpha <- runif(1)
  ...
  C_dgemmTN_coo(alpha, spA, B, beta, C)
  expect_equal(C, z)
})

test_that("blas coo dgemmNT", {
  m <- 20
  n <- 10
  k <- 5
  alpha <- runif(1)
  ...
  C_dgemmNT_coo(alpha, spA, B, beta, C)
  expect_equal(C, z)
})

test_that("blas coo dgemmTT", {
  m <- 20
  n <- 10
  k <- 5
  alpha <- runif(1)
  ...
  C_dgemmTT_coo(alpha, spA, B, beta, C)
  expect_equal(C, z)
})

test_that("blas coo dgemmNN2", {
  m <- 20
  n <- 10
  k <- 5
  alpha <- runif(1)
  ...
  C <- matrix(runif(m*n, min = -10, max = 10), m, n)
  z <- alpha * (A %*% B) + beta * C
  C_dgemmNN2_coo(alpha, A, spB, beta, C)
  expect_equal(C, z)
})

test_that("blas coo dgemmTN2", {
  m <- 20
  n <- 10
  k <- 5
  alpha <- runif(1)
  ...
  C <- matrix(runif(m*n, min = -10, max = 10), m, n)
  z <- alpha * (t(A) %*% B) + beta * C
  C_dgemmTN2_coo(alpha, A, spB, beta, C)
  expect_equal(C, z)
})

test_that("blas coo dgemmNT2", {
  m <- 2
  n <- 1
  k <- 5
  alpha <- 1 #runif(1)
  ...
  C <- matrix(runif(m*n, min = -10, max = 10), m, n)
  z <- alpha * (A %*% t(B)) + beta * C
  C_dgemmNT2_coo(alpha, A, spB, beta, C)
  expect_equal(C, z)
})

test_that("blas coo dgemmTT2", {
  m <- 2
  n <- 1
  k <- 5
  alpha <- 1 #runif(1)
  ...
  C <- matrix(runif(m*n, min = -10, max = 10), m, n)
  z <- alpha * (t(A) %*% t(B)) + beta * C
  C_dgemmTT2_coo(alpha, A, spB, beta, C)
  expect_equal(C, z)
})
